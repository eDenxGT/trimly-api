...
let ForgotPasswordUseCase = class ForgotPasswordUseCase {
    ...
    execute(_a) {
        return __awaiter(this, arguments, void 0, function* ({ email, role, }) {
            ...
            try {
                yield this._redisTokenRepository.storeResetToken((_b = user.userId) !== null && _b !== void 0 ? _b : "", resetToken);
            }
            catch (error) {
            ...
            const resetUrl = new URL(`${rolePrefix}/reset-password/${resetToken}`, config_1.config.cors.ALLOWED_ORIGIN).toString();
            yield this._emailService.sendResetEmail(email, "Trimly - Reset your password", resetUrl);
        });
    }
};
