...
let AddHairstyleUseCase = class AddHairstyleUseCase {
    ...
    async execute({ faceShapes, gender, name, image, }) {
        const hairstyle = await this._hairstyleRepository.findOne({
            name,
        });
        if (hairstyle) {
            throw new CustomError(ERROR_MESSAGES.HAIRSTYLE_ALREADY_EXISTS, HTTP_STATUS.BAD_REQUEST);
        }
...
AddHairstyleUseCase = __decorate([
    injectable(),
    __param(0, inject("IHairstyleRepository")),
    __metadata("design:paramtypes", [Object])
], AddHairstyleUseCase);
export { AddHairstyleUseCase };
