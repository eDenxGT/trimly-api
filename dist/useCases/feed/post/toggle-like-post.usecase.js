...
let ToggleLikePostUseCase = class ToggleLikePostUseCase {
    ...
    execute(_a) {
        return __awaiter(this, arguments, void 0, function* ({ postId, userId, }) {
            let post = yield this._postRepository.findOne({ postId });
            if (!post) {
                throw new custom_error_1.CustomError(constants_1.ERROR_MESSAGES.POST_NOT_FOUND, constants_1.HTTP_STATUS.NOT_FOUND);
            }
            let liked = (post === null || post === void 0 ? void 0 : post.likes.includes(userId)) || false;
            if (liked) {
                post = yield this._postRepository.removeLike({ postId, userId });
            }
            else {
                post = yield this._postRepository.addLike({ postId, userId });
            }
            return !liked;
        });
    }
...
exports.ToggleLikePostUseCase = ToggleLikePostUseCase = __decorate([
